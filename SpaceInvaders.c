...
#define MAX_ENEMYX MAX_X-ENEMY10W
...
uint8_t score=0;
int btn;
bool change_expression;

void GPIOPortD_Handler(void){
	Delay100ms();
	if(GPIO_PORTD_RIS_R&0x01){ //sw2 Start the game, change status to ON
		//Software acknowledges the flag and clears the read-only RIS bit by setting ICR
		GPIO_PORTD_ICR_R = 0x01;
		game_s=ON;
	}
	
	if(GPIO_PORTD_RIS_R&0x02){ //sw1 shoot a bullet
	  //Software acknowledges the flag and clears the read-only RIS bit by setting ICR
...
void Start_Prompt(void){
	Nokia5110_Clear();
  Nokia5110_OutString("             Space       Invaders    Press SW2   To Start");
  Delay100ms();                 // for board: delay ~1 sec at 80 MHz
}
...
void End_Prompt(void){
		delay = 0;
		Nokia5110_Clear();
		Nokia5110_OutString(" Game Over   Nice Try!   Your Score: 2 ");
		while(delay<30){};
}
...
void Switch_Init(void){
	...
	GPIO_PORTD_AMSEL_R &= ~0x03;     // 3) disable analog function
	//GPIO_PORTD_PUR_R |= 0x03;     //     enable weak pull-up on PF4
  GPIO_PORTD_PCTL_R &= ~0x000000FF; //  configure PF4,0 as GPIO
  GPIO_PORTD_DIR_R &= ~0x03;         // 6) PD0 and PD1 input
  GPIO_PORTD_AFSEL_R &= ~0x03;      // 7) no alternate function     
  GPIO_PORTD_DEN_R |= 0x03;         // 8) enable digital pins PD0 and PD1
	GPIO_PORTD_IS_R &= ~0x03; // Edge Sensitive
	GPIO_PORTD_IBE_R &= ~0x03; //Not both edges
	GPIO_PORTD_IEV_R &= ~0x03; // Falling Edge event
	GPIO_PORTD_ICR_R |= 0x03; //clear flag
	GPIO_PORTD_IM_R |= 0x03;      // (f) arm interrupt on PD0 and PD1
	NVIC_PRI7_R = (NVIC_PRI0_R&0x1FFFFFFF)|0x00400000; // priorty 4
	NVIC_EN0_R |= 0x08;
}
